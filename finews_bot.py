...
# ETF资金流向监控
ETF_FLOW_TICKERS = ["QQQ", "NVDA", "TSLA", "BTC-USD", "ETH-USD", "SOXL", "YINN", "IAUM"]
...
def build_radar_table(market_data: dict, ai_signals: dict) -> str:
    lines = [
        "```",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"{'标的/现价':<16} {'52周位置':<10} {'买入吸引力':<14} {'AI信号':<8} 下次财报",
        "─────────────────────────────────────────────────────────────",
    ]
    for symbol, info in market_data.items():
        signal   = ai_signals.get(symbol, "🟡 中性")
        col1     = f"{symbol} {info['price']}"
        line = f"{col1:<16} {info['position']:<10} {info['stars']:<14} {signal:<8} {info['earnings']}"
        lines.append(line)
        lines.append("")
    lines += [
        "─────────────────────────────────────────────────────────────",
...
给出明确信号 [偏多] [偏空] [中性]，没有相关新闻就跳过）

第三层：各市场之间怎么联动的
（今天最明显的跨资产传导链，比如油价涨了→通胀预期升→美债收益率涨→科技股压力→BTC跌）
